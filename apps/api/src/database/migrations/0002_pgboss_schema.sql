-- The pg-boss schema, generated by getConstructionPlans('pgboss') from
-- pg-boss 12.27.0 and committed like any other migration.
--
-- pg-boss can build this itself on boot. That is turned off (migrate: false,
-- createSchema: false) so schema changes stay a reviewed step and instances do
-- not race to alter tables on deploy - see DECISIONS.md.
--
-- Upgrading pg-boss needs a new migration from getMigrationPlans(), never an
-- edit to this file.
--
-- The outer BEGIN/COMMIT pg-boss emits is stripped: drizzle-kit already runs
-- each migration in a transaction, and the nested COMMIT would end it early.
